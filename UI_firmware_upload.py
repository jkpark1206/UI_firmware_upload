...
class upload():#步骤1：定义上传固件方法
    ...
    def firmware_info_input(self, file: object, task_name: object, version: object, firm: object):
        self.driver.get("http://{}".format('192.168.5.50:8011/#/'))
        self.driver.find_element(By.XPATH, '//input[@placeholder="请输入您的账号"]').send_keys('lmn')
        ...
        WebDriverWait(self.driver, 10).until(EC.text_to_be_present_in_element(A, '添加任务'))
        self.driver.find_element(By.CSS_SELECTOR,
        ...
        #等待元素可见
        WebDriverWait(self.driver, 10).until(EC.visibility_of_element_located(C))
        #点击添加任务按钮
        ...
        WebDriverWait(self.driver,30).until(EC.visibility_of_element_located(task))
...
def csv_file(): #读取firmware_list1.csv CSV文件，将固件全部上传，以firmware_list1.csv文件为例
    ...
    fp = open('text2.csv', 'r', encoding='GBK')
...
# import csv
# l=[]
# for filename in os.listdir(r'E:\\易识\\网关、采集器固件（含内蒙古电力项目）\\内蒙古电科项目客户提供\\上面文件解压后提取的固件汇总'):
#     l.append('E:\\易识\\网关、采集器固件（含内蒙古电力项目）\\内蒙古电科项目客户提供\\上面文件解压后提取的固件汇总\\'+filename)
#
# f=open("text2.csv","w")
# for line in l:
#     f.write(line+'\n')
